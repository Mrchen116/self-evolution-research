...
│   - dgm_metadata.jsonl 追加写入：本代尝试了谁、哪些子代过门槛、最新 archive   │
└──────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       ▼
┌──────────────────────────────────────────────────────────────────────────────┐
...
这一步看起来让模型产出了一大串分析字段，但真正进入下游执行的只有两块：
...
- `log_summarization / potential_improvements / improvement_proposal` 主要是给诊断模型自己组织思路
- 真正驱动 coding agent 改代码的是 `implementation_suggestion + problem_description`

所以这里的 JSON 不是最终产物，本质上只是把“反思”整理成一份可执行 issue。
...
满足这三个条件，节点就会被放进“可入 archive”的候选集。这里的 `compiled` 实际意思是：

> 这个子代至少成功走完了 patch 生成 + benchmark 评测的最小闭环。
